...
class SimpleCoinGeckoFetcher:
    ...
    def fetch_coingecko_price(self, crypto: str) -> float:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"CoinGecko API错误，状态码: {response.status_code}")
                return 0.0
    ...
    def fetch_all_prices_for_crypto(self, crypto: str) -> Dict[str, float]:
        """获取币种在多个交易所的价格"""
        base_price = self.fetch_coingecko_price(crypto)
...
# 全局实例
simple_coingecko_fetcher = SimpleCoinGeckoFetcher()


if __name__ == "__main__":
    # 测试代码
    cryptos = ["BTC", "ETH", "SOL", "USDT", "USDC"]
    logger.info("🧪 测试简单CoinGecko价格获取器...")

    prices = simple_coingecko_fetcher.fetch_all_prices(cryptos)

    print(f"\n=== 真实价格分析 ===")
    for crypto, crypto_prices in prices.items():
        if crypto_prices:
            analysis = simple_coingecko_fetcher.analyze_price_diff(crypto, crypto_prices)
            print(f"\n{crypto}:")
    ...
    opportunities = simple_coingecko_fetcher.get_all_opportunities(cryptos, prices)
    print(f"\n🚀 发现 {len(opportunities)} 个套利机会:")
    for i, opp in enumerate(opportunities, 1):
        print(f"  {i}. {opp['crypto']}: {opp['diff_rate']:.3f}% 利润 (${opp['potential_profit']:.2f})")

    # 显示市场总览
    summary = simple_coingecko_fetcher.get_market_summary()
    print(f"\n📊 市场总览:")
